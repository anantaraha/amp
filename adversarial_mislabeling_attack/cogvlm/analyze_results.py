...
Statistics/decision rules follow the LLaVA reference; objective metrics use this model's actual attacked output.
...
import numpy as np
import pandas as pd
from _vision import MODEL_ID, MODEL_REVISION, ATTACK_OUTPUT
...
def load_results(path, frame=None):
    frame = pd.read_csv(path, dtype="string") if frame is None else frame.copy()  # Preserve leading zeros in all image IDs.
    ...
    # FP32 cosine reduction can exceed an endpoint by a few ulps. Preserve the
    # measured values/statistics, but reject violations beyond numerical roundoff.
    if (frame[["R", "T", "B"]].abs() > 1 + CHANGE_EPS).any().any():
        raise ValueError("Cosines must lie in [-1, 1] within numerical tolerance.")
...
def measure_attack_output(path, encoder_results, bootstrap_reps, seed):
    """Apply the same cosine/gain/bootstrap rules to the ACTUAL author attack output.

    A private singleton index is used only to reuse the statistical functions;
    this representation is never added to encoder layers or depth groups.
    """
    raw = pd.read_csv(path, dtype="string")
    if "representation" not in raw or "attack_output" not in raw:
        raise ValueError(f"Expected the measured attack-output CSV: {path}")
    if not raw.representation.eq("attack_output").all() or not raw.attack_output.eq(ATTACK_OUTPUT).all():
        raise ValueError("Incorrect model/representation in the attack-output CSV")
    raw["layer"] = "0"
    frame, _ = load_results(path, frame=raw)
    reference = encoder_results.drop_duplicates("sample_id").set_index("sample_id")
    actual = frame.set_index("sample_id")
    if set(reference.index) != set(actual.index):
        raise ValueError("Encoder and attacked-output metrics do not cover identical samples")
    for column in ["pair_id", "source_image_id", "target_image_id", "source_concept", "target_concept"]:
        if column not in actual and column not in reference:
            continue
        if column not in actual or column not in reference:
            raise ValueError(f"Missing attacked-output provenance: {column}")
        if not actual.loc[reference.index, column].fillna("").equals(reference[column].fillna("")):
            raise ValueError(f"Attacked-output provenance mismatch: {column}")
    summary = summarize(frame, ["layer"]).merge(
        bootstrap_intervals(frame, bootstrap_reps, seed), on="layer", validate="one_to_one")
    record = summary.iloc[0].drop(labels=["layer"]).to_dict()
    record.update(objective_representation="attack_output", attack_output=ATTACK_OUTPUT,
                  available=True, total_samples=encoder_results.sample_id.nunique(),
                  M_min=frame.M.min(), M_max=frame.M.max())
    denominator = 1 - frame.B
    frame["normalized_target_distance_reduction"] = frame.G.div(denominator.where(denominator > DISTANCE_EPS))
    ratios = frame.normalized_target_distance_reduction.dropna()
    record.update(distance_reduction_valid=len(ratios), distance_reduction_excluded=len(frame)-len(ratios))
    for stat in ["mean", "median", "min", "max"]:
        record[f"distance_reduction_{stat}"] = getattr(ratios, stat)() if len(ratios) else np.nan
    pairs = summarize(frame, ["pair_id"]) if "pair_id" in frame else pd.DataFrame()
    pairs["coverage_pct"] = 100.0  # Complete matching was checked above.
    return {"exp1_objective_summary": pd.DataFrame([record]),
            "exp1_attack_output_pair_summary": pairs,
            "exp1_attack_output_metrics": frame.drop(columns="layer")}


def validate_experiment_metadata(exp1_csv, exp2_dir):
    """Prevent combining different models, revisions or representation layouts."""
    import json

    records = []
    for directory in [Path(exp1_csv).parent, Path(exp2_dir)]:
        path = directory / "representation_metadata.json"
        metadata = json.loads(path.read_text())
        if metadata["model_id"] != MODEL_ID or metadata["model_revision"] != MODEL_REVISION:
            raise ValueError(f"Different model/revision in {path}")
        records.append(metadata)
    if records[0] != records[1]:
        raise ValueError("Exp1 and Exp2 representation metadata do not match")


def measure_exp1(exp1_csv, bootstrap_reps=BOOTSTRAP_REPS, seed=SEED, attack_output_csv=None):
    ...
    path = Path(attack_output_csv) if attack_output_csv is not None else Path(exp1_csv).with_name("exp1_attack_output_cosine.csv")
    objective_tables = measure_attack_output(path, results, bootstrap_reps, seed)
    return {
...
def measure_all(exp1_csv, exp2_dir, bootstrap_reps=BOOTSTRAP_REPS, seed=SEED, attack_output_csv=None):
    """Finish every measurement before any finding is constructed."""
    validate_experiment_metadata(exp1_csv, exp2_dir)
    tables = measure_exp1(exp1_csv, bootstrap_reps, seed, attack_output_csv)
...
def supported_findings(tables):
    ...
    if objective.available:
        ...
        findings.append(f"At the measured attack output ({ATTACK_OUTPUT}), the consistent-gain rule "
        ...
        pairs = tables["exp1_attack_output_pair_summary"]
        if not pairs.empty:
            unanimous_pairs = ((pairs.M_positive_pct == 100) & (pairs.coverage_pct == 100)).sum()
            findings.append(f"At the measured attack output, {int((pairs.G_mean > 0).sum())}/{len(pairs)} observed concept pairs have positive mean G, "
        ...
        findings.append("Measured attack output is absent; no objective-output finding is generated.")
...
def build_report(tables, exp1_csv, exp2_dir):
    ...
    lines = [
        ...
        *block_text,
        ...
        f"The AMP objective output is measured directly: {ATTACK_OUTPUT}. It is separate from encoder states and depth groups.",
    ]
    if objective.available:
        row = objective
        lines.append(f"Measured attack output: {int(objective.n_samples)}/{int(objective.total_samples)} samples; "
        ...
        lines.append("Measured attack output: absent.")
...
def parse_args():
    ...
    parser.add_argument("--attack-output-results", type=Path, default=None,
                        help="Actual attack-output cosine CSV; omitted uses exp1_attack_output_cosine.csv beside --exp1-results.")
...
def main():
    args = parse_args()
    tables = measure_all(args.exp1_results, args.exp2_dir, args.bootstrap_reps, args.seed, args.attack_output_results)
