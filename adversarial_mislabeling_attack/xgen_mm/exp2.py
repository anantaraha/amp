#!/usr/bin/env python3
"""xGen-MM dataset-level CKA using all-token means of native encoder states.
...
def run_experiment(args):
    ...
    try:
        draw_cka_heatmap(
            axes[0, 0], cka_source,
            f"xGen-MM source linear CKA (N={valid_count} AMP samples)",
            "viridis", 0, 1, "Linear CKA", labels,
        )
        draw_cka_heatmap(
            axes[0, 1], cka_adversarial,
            f"xGen-MM adversarial linear CKA (N={valid_count} AMP samples)",
            "viridis", 0, 1, "Linear CKA", labels,
        )
        draw_cka_heatmap(
            axes[1, 0], cka_target,
            f"xGen-MM target linear CKA (N={valid_count} AMP samples)",
            "viridis", 0, 1, "Linear CKA", labels,
        )
        draw_cka_heatmap(
            axes[1, 1], cka_delta,
            f"xGen-MM adversarial − source CKA (N={valid_count} AMP samples)",
